...
public class QueryReplicator extends Replicator {

    public static final String QUERY_STATUS_MESSAGE_TYPE = "deltix.qsrv.hf.tickdb.lang.pub.messages.QueryStatusMessage";
    ...
    private final RawMessageHelper messageHelper = new RawMessageHelper();

    public QueryReplicator(QueryRequest queryRequest,
                           DXTickDB tickDb, ClickhouseClient clickhouseClient,
    ...
    private SchemaOptions getSchemaOptions() {
        ClassSet<?> classSet =  tickDb.describeQuery(request.getQuery(), new SelectionOptions(true, false));
        ...
        RecordClassSet recordClassSet = new RecordClassSet(contentClasses);

        Map<String, String> mapping;
        if (!request.getTypeTableMapping().isEmpty()){
            mapping = request.getTypeTableMapping();
        } else if (request.isSplitByTypes()) {
            mapping = Arrays.stream(recordClassSet.getTopTypes())
                    .map(NamedDescriptor::getName)
                    .collect(Collectors.toMap(name -> name, Util::getSimpleName));
        } else {
            mapping = new HashMap<>(){{put(ALL_TYPES, request.getTable());}};
        }

        return new SchemaOptions(recordClassSet, mapping, request.getWriteMode(), request.getColumnNamingScheme(), request.getIncludePartitionColumn());
    }
    ...
    public boolean equals(Object o) {
        ...
        QueryReplicator that = (QueryReplicator) o;
